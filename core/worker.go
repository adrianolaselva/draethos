...
func (s *worker) Start() error {
	...
	source, err := context.NewSourceContext(s.configSpec, target, dlq, s.router, s.configSpec.Stream.Port)
	if err != nil {
		return err
	}
...
func (s *worker) Setup() {
	...
	if s.configSpec.Stream.Instance.Source.Type != context.HttpSource {
		srv := &http.Server{
			Handler:      s.router,
